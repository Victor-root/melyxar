...
- **Un fichier n'est traversé qu'une fois.** Direction voulue par le mainteneur, dans l'esprit de Jellyfin : prendre d'un fichier tout ce qu'il peut donner en une seule lecture, au lieu d'enchaîner des passes qui rouvrent chacune les mêmes films. **En bonne voie** : les traversées ne sont plus des passes du scan mais des travaux de l'entretien, ce qui règle le coût pour celui qui lance un scan, et la lecture de l'index du conteneur a supprimé l'une des deux pour tous les formats courants. Il reste la traversée des vignettes, et c'est dans celle-là que les sous-titres à extraire doivent voyager.
