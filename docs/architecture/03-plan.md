...
- **Extraction des sous-titres au scan.** Sortir une piste de sous-titres d'un film demande de lire le fichier entier, parce que les mots sont entrelacés avec l'image d'un bout à l'autre : mesuré à trois quarts de minute sur un film 4K. Demandée au moment du clic, cette attente tombe entièrement sur quelqu'un qui vient d'appuyer et ne voit rien venir. Elle est pour l'instant lancée dès l'ouverture du film, ce qui la déplace hors du chemin, mais la bonne réponse est de convertir les pistes une fois pour toutes pendant le scan, dans la traversée dont il est question ci-dessous.
- **Un fichier n'est traversé qu'une fois.** Direction voulue par le mainteneur, dans l'esprit de Jellyfin : un scan prend d'un fichier tout ce qu'il peut en une seule lecture, au lieu d'enchaîner des passes qui rouvrent chacune les mêmes films. Aujourd'hui le relevé des images clés lit déjà la collection entière de bout en bout ; coder les vignettes sans y penser en ferait une deuxième traversée complète, pour la même donnée au même instant. Les deux issues sont ci-dessus : lire l'index du conteneur rend la question sans objet pour les images clés, et ce qui reste à extraire voyage dans la même traversée. À décider avant d'écrire les vignettes, pas après.
...
- **Paramètres de chaque médiathèque, dont la langue de ses fiches**, avec « suivre la langue de l'administrateur » comme valeur par défaut. En changer relance l'identification sur toute la médiathèque, comme une tâche de fond ordinaire. Aujourd'hui cette langue est lue dans le fichier de configuration à la seule création de la médiathèque, son défaut est le français, et rien ne permet d'en changer : une installation anglophone reçoit des fiches en français et ne peut pas en sortir, même en éditant le fichier. Les deux petits correctifs (un défaut qui n'est plus le français, et un changement du fichier réellement appliqué à une médiathèque existante, suivi du re-téléchargement) peuvent partir avant l'écran lui-même.
...
- **Réglages des miniatures de la barre de lecture depuis l'interface** : intervalle, largeur de l'image et découpage des planches, comme le fait Jellyfin aujourd'hui, avec l'avertissement que changer la taille demande de refaire les planches de toute la médiathèque. Ces valeurs existent déjà dans le fichier de configuration : ce qui manque est l'écran, et le fait qu'un changement relance la fabrication pour ce qui ne correspond plus.
