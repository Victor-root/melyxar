...
- **Surveillance en temps réel des médiathèques**, comme l'option « Activer la surveillance en temps réel » de Jellyfin : un interrupteur par médiathèque, et quand il est coché, un fichier ajouté, déplacé, renommé ou supprimé dans ses dossiers est traité aussitôt, sans attendre un scan. Seulement sur les systèmes de fichiers qui préviennent d'un changement ; ailleurs l'interrupteur dit pourquoi il ne peut rien. Demandé par le mainteneur. À vérifier avant d'écrire la décision : que les disques montés dans le conteneur non privilégié préviennent bien d'un changement, et comment traiter un film encore en cours de copie.
