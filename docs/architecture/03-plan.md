...
- **Ajouter et retirer une médiathèque depuis l'interface**, en parcourant l'arborescence du serveur pour désigner ses dossiers. C'est la dernière chose qui exige encore d'ouvrir le fichier de configuration. Demande une décision avant d'être codée : accepter un chemin venant du client, ce que le dépôt interdit partout ailleurs. Le reste des paramètres d'une médiathèque est fait : la langue de ses fiches, et les deux interrupteurs disant ce que son scan lit lui-même.
