...
- **Un fichier n'est traversé qu'une fois.** Direction voulue par le mainteneur, dans l'esprit de Jellyfin : prendre d'un fichier tout ce qu'il peut donner en une seule lecture, au lieu d'enchaîner des passes qui rouvrent chacune les mêmes films. **Toujours à faire, et déplacé** : les deux traversées ne sont plus des passes du scan mais les deux travaux de l'entretien, ce qui règle le coût pour celui qui lance un scan et pas le coût total. Il reste donc deux lectures intégrales par film, plus une troisième le jour où les sous-titres seront extraits au scan. Les deux issues sont ci-dessus : lire l'index du conteneur rend la question sans objet pour les images clés, et ce qui reste à extraire voyage dans la même traversée que les vignettes.
