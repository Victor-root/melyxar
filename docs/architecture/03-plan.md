...
- Séries : ranger à nouveau des épisodes déjà rangés quand les règles s'améliorent. Un film qui attend son nom est relu à chaque scan ; un épisode déjà accroché à une série ne l'est pas, parce que son fichier n'a pas bougé. Aujourd'hui la seule façon de profiter d'une meilleure lecture sur une collection déjà scannée est de retirer la médiathèque et de la remettre, ce qui ne touche à aucun fichier mais fait tout redemander au fournisseur.
