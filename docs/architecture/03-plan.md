...
- **Surveillance en temps réel des médiathèques. Fait.** Un interrupteur par médiathèque dans l'administration, décoché par défaut. Coché, un fichier ajouté, déplacé, renommé ou supprimé relance le scan ordinaire de la médiathèque dès que ses dossiers sont calmes : dix secondes sans changement et des fichiers dont la taille ne bouge plus, donc un film en cours de copie n'est pris qu'une fois la copie finie. La lecture d'un film, son ouverture et ses dates d'accès ne comptent pas comme des changements, pas plus que les fichiers cachés. Un scan déjà en cours n'est pas doublé. Quand la surveillance est impossible (trop de dossiers pour la limite du système, dossier absent), le panneau le dit avec la raison, et le serveur réessaie toutes les cinq minutes. Vérifié contre un serveur qui tourne : une copie lente de vingt secondes n'a déclenché qu'un seul scan, après la fin de la copie ; une suppression est vue ; la lecture d'un film ne déclenche rien. Un fichier **renommé** est suivi comme un fichier déplacé, par le scan ordinaire comme par la surveillance : il est reconnu à sa taille à l'octet près et à sa date de modification à la nanoseconde près, et garde la position de lecture, les vus, les favoris et la liste à voir. Quand deux fichiers pourraient correspondre, rien n'est touché.
