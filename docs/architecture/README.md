...
| Versions multiples | Le modèle garde plusieurs sources par œuvre, mais aucun sélecteur ni choix automatique n'est construit | Les sources multiples servent au repérage des remplacements et des doublons, pas à un choix de qualité. |
...
| Clavier | Vrais boutons et vrais liens, ordre de tabulation respecté, contour visible sur l'élément sélectionné, dès le premier composant. Navigation directionnelle non planifiée | Le niveau gratuit est très coûteux à rattraper, le niveau directionnel ne l'est pas. |
