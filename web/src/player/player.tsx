 ...
 *
 * Two parts, because they do not live as long as each other. The player
 * itself stays for as long as somebody is watching: the screen, whether it is
 * filled, how the words are dressed. The film inside it is one file played,
 * from asking the server what to do with it to leaving it, and it starts
 * afresh with every file: an episode following another is a new film in the
 * same player, and nothing the last one knew about itself carries over.
 *
...
/**
 * The player, for as long as somebody is watching.
 *
 * `work` is absent for the moment between one episode and the next while the
 * description of the next is on its way: the player stays, filled screen and
 * all, and shows that something is coming.
 */
export function Player(props: Omit<Watched, "work"> & { work: Work | null }) {
  ...
  return (
    <div className={`player ${appearanceClasses(appearance)}`} role="dialog" aria-label={work?.title}>
      {/* What is sent fullscreen: the picture and everything drawn over it. */}
      <div className="player-stage" ref={stage}>
        {work ? (
          <Film
            /* A fresh film for every file. What a film holds about itself
               (where it had got to, what the server said to do with it, the
               session it is fed from) belongs to that file alone: carried
               over, the next episode opened where the last one ended, reached
               its end at once and stepped again. */
            key={sourceId}
            {...props}
            work={work}
            stage={stage}
...
/** One file played, from asking the server what to do with it to leaving it. */
function Film({
  sourceId,
  work,
  fromTheStart,
